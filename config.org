...
Load Solaire before loading the theme, so that Solaire is actually
enabled.
...
To control the screen brightness, we need to install the ~brightnessctl~
package.
...
For polybar we need [[https://aur.archlinux.org/packages/ttf-font-awesome-4][ttf-font-awesome-4]].
...
We setup transient states for manual window management.
#+begin_src emacs-lisp
